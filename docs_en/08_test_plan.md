...
| TC-108 | FEAT-006 Render review & moderation | NFR-008 | An operator without the `render_reviewer` role (or no operator session) attempts to approve or reject a render | The action is rejected (403 without the role; 401 without a session) and the render's review status is unchanged | Retired — ADR-025 (was Automated #34). As-built 2026-07-15: the render-review endpoint and its automated test are already removed — `routes.test.ts` asserts the operator render-review routes are NOT registered, and `hardening.test.ts` records TC-108 as retired; no `render_reviewer` role exists (schema `OperatorRole` = `catalog_curator`/`order_handler`). |
...
| TC-120 | FEAT-002 Room capture & inputs | FR-005 | The owning device uploads raw image bytes (`image/jpeg\|image/png\|image/webp`) to `POST /projects/:id/photos` *(as-built byte upload — FEAT-002)* | The bytes are stored in object storage under `rooms/<projectId>/<uuid>.<ext>` and a `RoomPhoto` row is created with that `storageKey` (`qualityStatus` `pending`); the response is `201 { id, storageKey }` | Pending |
| TC-121 | FEAT-002 Room capture & inputs | NFR-007 | A foreign/unknown device token (or a project not owned by the device) uploads a photo to `POST /projects/:id/photos` | The upload is refused with `404`; no bytes are stored and no `RoomPhoto` is created (device scoping) | Pending |
| TC-122 | FEAT-005 AI rendering engine | FR-015 | The owning device calls `GET /renders/:id/image` for a completed render that has an `imageKey` *(as-built render display — FEAT-002)* | The stored render bytes are returned with an image content-type (`image/png`) | Pending |
| TC-123 | FEAT-005 AI rendering engine | NFR-007 | A foreign/unknown device token calls `GET /renders/:id/image` | The request is refused with `404` and no render bytes are leaked (device scoping) | Pending |
| TC-124 | FEAT-005 AI rendering engine | FR-015 | The owning device calls `GET /renders/:id/image` for a render with no `imageKey` yet (still generating or failed) | A `404` (or `409`) is returned so the client keeps polling; no bytes are returned | Pending |
| TC-125 | FEAT-002 Room capture & inputs | FR-005 | A non-image `Content-Type` or an empty body is sent to `POST /projects/:id/photos` | The upload is rejected with `400`; no bytes are stored and no `RoomPhoto` is created | Pending |
| TC-126 | FEAT-005 AI rendering engine | FR-015 | A large room photo (e.g. a ~24 MP phone photo) is rendered *(BUG-001)* | Before mflux runs, every input image is downscaled to the configured max longest edge with EXIF orientation baked in, so the render completes without a GPU out-of-memory crash and is not rotated | Pending |
| TC-127 | FEAT-018 Browse & select furniture | FR-066 | Browse the catalog with a `source` (`supplier`\|`public`) and a `styleId` that has approved, renderable matches *(ADR-028)* | The approved, renderable products of that source+style are returned, each with an `imageUrl` (public → the product-image endpoint; supplier → its web-asset `photos[0]`) | Pending |
| TC-128 | FEAT-018 Browse & select furniture | FR-066 | Browse when the source+style has unapproved/over-`budgetMaxCop` products, or no matching products at all | Unapproved, non-renderable, wrong-source/style, and over-budget products are excluded; when none match, an empty product list is returned | Pending |
| TC-129 | FEAT-018 Browse & select furniture | FR-066 | Call `GET /catalog/products/:id/image` for a public product with a stored image, and for one with no stored image or that does not exist | The stored image bytes are streamed with the correct content-type for the first; `404` is returned for the second | Pending |
| TC-130 | FEAT-018 Browse & select furniture | FR-067 | Submit `POST /renders` with more than 3 `productIds` *(3-item cap, ADR-028/ADR-026)* | The request is rejected server-side with `400 too_many_products` and no render is created | Pending |
| TC-131 | FEAT-018 Browse & select furniture | FR-067 | Submit `POST /renders` with `productIds` (≤3) that include a non-existent, unapproved, non-renderable, or out-of-stock ready-made product | The render is accepted (no 400) and the worker's selection validation silently drops the invalid id(s), compositing only the valid selected products | Pending |
| TC-132 | FEAT-018 Browse & select furniture | FR-068 | Submit `POST /renders` with a valid `productIds` selection | The render worker composites **exactly** the selected (validated) products; auto-match (`matchProducts()`/FR-014) is not run | Pending |
| TC-133 | FEAT-018 Browse & select furniture | FR-068 | Submit `POST /renders` with no `productIds` | Auto-match (FR-014/FR-015) runs as the fallback and the render behavior is unchanged (backward compatible) | Pending |
| TC-134 | FEAT-018 Browse & select furniture | FR-069 | After a completed render, select a different set of products (≤3) on the same project and re-render *("try other furniture")* | A new render compositing the new selection is produced for the same project, reusing the same photo, source, and style | Pending |
| TC-135 | FEAT-018 Browse & select furniture | FR-068 | Render a Brand (`source=public`) selection, then a Local (`source=supplier`) selection, and inspect the resulting cart *(guard — FR-064/FR-031, ADR-027)* | The Brand/public render populates no cart items (display-only, empty cart by design); the Local/supplier render auto-populates the cart with the selected SKUs | Pending |
| TC-136 | FEAT-005 AI rendering engine | FR-015 / NFR-004 | The mflux render child hangs and never exits (e.g. the render host is out of RAM and the model never loads) *(BUG-002 hard timeout)* — unit test with a fake spawner whose child never emits `close` and a tiny `timeoutMs` | The pipeline SIGKILLs the child at the hard cap (`RENDER_TIMEOUT_MS`) and rejects with a "render timed out" error; the worker marks the `RenderRequest` `failed`, so a request never stays `processing` forever and no orphan child survives past the cap (graceful degradation) | Pending |
| TC-137 | FEAT-005 AI rendering engine | NFR-004 / NFR-007 | The owning device calls `POST /renders/:id/cancel` on an in-flight render; then the foreign/unknown-device and already-terminal cases *(BUG-002 cancel)* | Owning device: any live child is SIGKILLed via the render registry and the request is marked `failed` (`200 { status: "failed" }`). A foreign/unknown device or unknown render → `404`, nothing killed or updated (device scoping). Cancelling an already-terminal render → `200` with no status write (idempotent) | Pending |
| TC-138 | FEAT-018 Browse & select furniture | FR-015 / NFR-004 | Run the full flow (room → style → select 3 → render) on the dev server, where React Strict Mode double-invokes effects *(BUG-003 strict-safe kickoff)* — scripted browser click-through | The render page advances loading → generating → **ready** and displays the render; it never sticks on the spinner while the backend render completes unseen (the pre-fix behavior: the Strict Mode remount discarded the kickoff's phase transition, no poll ever ran) | Pending |
| TC-139 | FEAT-018 Browse & select furniture | FR-015 / NFR-004 | Reload the browser on `/render` while a render is generating or after it completed; also reload on `/select` *(BUG-003 sessionStorage persistence + resume)* | The wizard rehydrates from sessionStorage: `/render` **resumes polling the same render** (same `renderId` — no duplicate submission, no bounce to `/room`) and shows it when completed; `/select` reloads its catalog in place. Guards wait for rehydration before redirecting | Pending |
| TC-140 | FEAT-018 Browse & select furniture | FR-069 | After a completed render, use "Try other furniture" and re-pick (even the same) products; after a failed render, retry the same selection *(BUG-003 reset-render semantics)* | Both paths submit a **fresh** render (new `renderId`) instead of resuming the previous one — a failed render is never resumed into an error loop, and iterating re-renders rather than replaying the old result | Pending |
