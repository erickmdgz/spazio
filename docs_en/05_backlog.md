...
- **Human decisions.** Several features depend on choices reserved for humans in PRD §12 and tracked as ADRs in `/docs_en/decisions/` (technology stack, AI/rendering pipeline, payment gateway and split settlement, merchant-of-record, style taxonomy, commission %, cart-hold duration, daily free-render limit, render-package pricing, budget tolerance, catalog-sync frequency, render-time target, launch markets, supplier partners, taxes/compliance, privacy, warranty/disputes, brand). These features are **specifications pending those decisions**, not commitments to a value.
