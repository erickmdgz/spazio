...
These are **quality-attribute specifications**. *(Correction, 2026-07-15: the former blanket "Nothing here is built yet" is no longer accurate. Several of these constraints are **exercised by the current system, built and verified on a local stack (not deployed, no `vX.Y.Z` release)** — notably NFR-007 (photos/renders private and device-scoped; a foreign device gets 404 — enforced on the as-built photo-upload and render-image routes, and strengthened because the self-hosted engine keeps images on Spazio infrastructure), NFR-006 (the render-to-purchase **event trail** records from day one; the metric *computation* and the ADR-027 public-render segmentation remain to be built — TC-119 pending), and NFR-019 (CC BY 4.0 attribution for public-catalog data/images). Others (performance SLAs, cost thresholds, split settlement, multi-currency, guest checkout) remain specification only. See the "Current system (as built)" note in `03_requirements.md`; each NFR's status/trace states what is exercised vs. still spec.)*
