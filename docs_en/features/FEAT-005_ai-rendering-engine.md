...
>
> **As built — render-job robustness & diagnosability (BUG-005, 2026-07-16).** Render jobs now run **strictly serialized** (an in-flight guard in `InMemoryQueue.drain` — two mflux children at ~20 GB MLX peak each would exhaust the render host; TC-142). A hard-timeout rejection now includes the **tail of the child's collected stderr** (previously discarded — the BUG-004 incident was undiagnosable from the bare "timed out" line; TC-143), the worker logs **job start** and the pipeline logs the **spawned mflux command**, and the requested selection is **snapshotted** on `RenderRequest.requestedProductIds` (additive migration) so a failed render keeps the attempted products (TC-144; `07_data_model.md`). No API-contract change; no new dependency. No new ADR — robustness fixes realizing NFR-004.
