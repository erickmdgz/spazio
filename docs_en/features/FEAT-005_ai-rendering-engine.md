...
> **Cross-ref — user-curated selection (ADR-028, 2026-07-15; FEAT-018).** The furnishing flow is inverted: the user **browses the catalog and selects up to 3 products**, and the render composites **exactly** that selection (FR-068) rather than the auto-matched set. The **matching side of this feature (FR-014) becomes an optional fallback**, run only when a render request carries no user selection (`productIds` omitted); FR-015 (composite generation) is unchanged in observable result. The 3-item cap matches this engine's ~2–3 reference-image limit (ADR-026). See `features/FEAT-018_browse-select-furniture.md`, FR-066..069.
...
>
> **As built — render hard timeout + cancel (BUG-002, 2026-07-15).** A hung mflux child (e.g. the render host out of RAM, model never loading) previously never exited, so the `RenderRequest` stayed `processing` forever, the `/render` page polled endlessly, and closing the tab left the child running as a memory-thrashing **orphan** — no timeout, no cancellation. Two backend safeguards now make a stuck/failed render fail **gracefully** and **stop the work** (enforced on the backend — a browser cannot kill a server process; NFR-004 graceful degradation): **(1) hard timeout + SIGKILL** — `MfluxRenderPipeline` races each mflux run against `RENDER_TIMEOUT_MS` (config, default `900000` = 15 min since BUG-004; BUG-002 shipped it as 6 min); on timeout it SIGKILLs the child and rejects with a "render timed out" error, and the worker marks the request **`failed`** (self-heals with no client — no request stays `processing` past the cap, no orphan survives it). **(2) cancel** — a new **device-scoped** `POST /api/v1/renders/:id/cancel` (NFR-007) SIGKILLs any live child via an in-process render registry (`services/render/registry.ts`) and marks a still-running request `failed`, letting the client stop an in-flight render immediately on its own backstop timeout and when leaving `/render`; **idempotent** (already-terminal → `200` no-op), foreign/unknown → `404`. **No schema/migration change** (cancellation reuses the existing `failed` status) and **no new dependency**. Contract in `06_api.md` (§5); tests **TC-136 / TC-137** (`08_test_plan.md`). No new ADR — a robustness fix realizing NFR-004, no new decision.
>
> **As built — render-job robustness & diagnosability (BUG-005, 2026-07-16).** Render jobs now run **strictly serialized** (an in-flight guard in `InMemoryQueue.drain` — two mflux children at ~20 GB MLX peak each would exhaust the render host; TC-142), and a job whose request is already terminal at dequeue (cancelled while it waited in the now long-lived `queued` state) is **skipped, never resurrected** to `processing` (TC-145; upholds TC-137). A hard-timeout rejection now includes the **tail of the child's collected stderr** (previously discarded — the BUG-004 incident was undiagnosable from the bare "timed out" line; TC-143, bounded accumulator), the worker logs **job start** and the pipeline logs the **spawned mflux command** (args quoted, copy-paste reproducible), and the requested selection is **snapshotted** on `RenderRequest.requestedProductIds` (additive migration) so a failed render keeps the attempted products (TC-144; `07_data_model.md`). No API-contract change; no new dependency. No new ADR — robustness fixes realizing NFR-004.
