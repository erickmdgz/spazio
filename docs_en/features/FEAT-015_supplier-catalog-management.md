...
Test cases live in `08_test_plan.md`, where **each `TC-` maps 1:1 to an acceptance criterion of an FR** (see `03_requirements.md`). The `TC-` rows for this feature's related FRs (FR-055–FR-060) **already exist** in `08_test_plan.md` (all with `Status: Pending`, since nothing is implemented) — they are **TC-093 through TC-105**:
