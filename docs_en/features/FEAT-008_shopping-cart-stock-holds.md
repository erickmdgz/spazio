...
> **Cross-ref — user-curated selection (ADR-028, 2026-07-15; FEAT-018).** The cart is auto-populated (FR-031) from the render's items, which are now the **up-to-3 products the user selected** (FR-068) rather than an auto-matched set. Provenance (ADR-027) governs what lands in the cart: a **Local suppliers** (`source=supplier`) selection populates the cart normally; a **Brand suppliers** (`source=public`) selection is **display-only** and yields an **empty cart by design** (FR-064) — the user buys via the "View at retailer" link. Cart behavior (FR-031/FR-032/FR-033/FR-035) is otherwise unchanged. See `features/FEAT-018_browse-select-furniture.md`.
