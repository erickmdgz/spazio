...
Test cases live in `08_test_plan.md`, where **each `TC-` maps 1:1 to an acceptance criterion of an FR** (see `03_requirements.md`). The `TC-` rows for this feature **already exist** in `08_test_plan.md` (status `Pending` — written, not yet executed): **TC-065–TC-068** (FR-036, FR-037, FR-038). **Build-status correction (2026-07-15):** the pilot slice FR-036 (per-item supplier-sourced production/delivery estimates before checkout) is **built and verified on a local stack** — `GET /api/v1/cart/estimates` returns each item's `deliveryLeadTimeDays` / `productionLeadTimeDays` (null when the supplier estimate is absent; the `missing-estimate` status of TC-066 remains a DRAFT/PROPOSED spec label, not yet emitted by the API), surfaced on the cart and confirmation — so the earlier "nothing here is implemented" boilerplate is superseded for FR-036; `Pending` there means **TCs pending automation**. FR-037 (aggregated order estimate) and FR-038 (warranty display) remain out of pilot/specification.
