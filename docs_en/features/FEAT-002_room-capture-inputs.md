...
> **As built — real byte upload (2026-07-15; branch `feature/FEAT-002-photo-upload-render-display`).** FR-005 is now implemented as a **real image-byte upload** so the render engine (ADR-026) composites the user's actual photo. `POST /api/v1/projects/:id/photos` accepts the raw file bytes (`Content-Type: image/jpeg|image/png|image/webp`, body = the bytes, **not** JSON) parsed by a Fastify `addContentTypeParser` with `parseAs: "buffer"` — **no new dependency** (no `@fastify/multipart`) — with the route `bodyLimit` raised to ~20 MB. The handler stores the bytes via `ObjectStorage.put` under `rooms/<projectId>/<uuid>.<ext>`, creates the `RoomPhoto` (`qualityStatus` `pending`) and returns `201 { id, storageKey }`. It is **device-scoped** (`requireDeviceToken` + project-owned-by-device; foreign/unknown device or unowned project → 404, NFR-007); a non-image content-type or empty body → 400; a body over ~20 MB → 413. The web app now retains the user's chosen `File` and sends its real bytes (with the file's MIME type) instead of substituting the preset "living" sample and posting a preset key string. **Upload is the primary, prominent action on `/room`;** the two prior illustration samples (`living-before.svg` / `bedroom-before.svg`) are replaced by a **single real sample photo** (`/rooms/sample-living.png`) under a clearly-labeled "No photo handy? Try a sample" fallback — its bytes are uploaded like any photo, so the engine composites a real image (an illustration would render poorly). The offline/while-generating fallback for the sample is a real pre-rendered mflux composite (`/rooms/sample-living-rendered.png`), not the old per-style illustration. Room **dimensions** stay on `PATCH /projects/:id` (FR-011), not in this body. Contract in `06_api.md` (§2); FR-005 as-built note in `03_requirements.md`; tests **TC-120 / TC-121 / TC-125** (`08_test_plan.md`). The **render-display** other half of this branch (a new `GET /renders/:id/image` serving the stored render) is recorded in FEAT-005 and FEAT-007 and under FR-015.
...
- FR-005 as-built byte upload (2026-07-15) — **TC-120** the owning device uploads raw image bytes and they are stored in object storage with a `RoomPhoto` created for that key; **TC-121** a foreign/unknown device (or unowned project) is refused `404` (NFR-007); **TC-125** a non-image content-type or empty body is rejected `400`.
