...
> **Status of this document.** This document mixes a full-product **specification** with a description of what is now **actually built**. A substantial slice is **built and verified on a local dev stack** — the web app end to end over the real backend — but **nothing is deployed or released** (no `vX.Y.Z` tag). Earlier text claiming "nothing of the system is implemented" described a superseded state and has been corrected; see the "Current system (as built)" note directly below and the [Class-demo architecture](#class-demo-architecture-web) section. The spec still structures how the full product is intended to work end to end and records the platform-level choices in the ADRs under `/docs_en/decisions`. **Update (ADR-024, 2026-07-14):** the client is the **web app** (no native iOS will be built) at class-demo scale — read the client boxes in the diagrams below as the web app. **Update (ADR-025, 2026-07-14):** the human render-review gate is removed — renders are published to the requesting user immediately upon successful generation (FR-027/FEAT-006 retired; the mandatory-operator-QA clause of ADR-002 superseded, the rest of ADR-002 stands); implemented by FEAT-016 (#38). **Update (ADR-028, 2026-07-15):** the furnishing flow is **user-curated browse-and-select** (FEAT-018), not AI auto-furnish — see the flow note under [Main modules](#main-modules).

> **Current system (as built) — 2026-07-15.** **Platform:** web app (`web-demo/`, Next.js 15 + React 19 + TS + Tailwind) → Node 22 / Fastify / Prisma / Postgres backend + local object storage. No native iOS. **Flow (FEAT-018/ADR-028):** landing → upload real room-photo bytes (large phone photos downscaled + EXIF-oriented, BUG-001) + approximate dimensions → choose **source** (Local suppliers `source=supplier` = purchasable | Brand suppliers `source=public` = display-only) → choose **style** (+ COP budget) → **browse the real catalog, select ≤3 products** → **render** the selection → "Love it → cart" (Local) / per-item "View at retailer" (Brand) / "Try other furniture" (iterate) → cart → **mock** checkout → confirmation. The ≤3 cap matches the render engine's ~2–3 reference-image limit (ADR-026). **Render engine (ADR-026):** self-hosted **FLUX.2 Klein 4B via the mflux CLI** as a child process on an **Apple-Silicon** render worker; `RENDER_ENGINE` defaults to `fake` (placeholder/cached, keeps CI hermetic), `mflux` runs the real engine; inputs downscaled to `MFLUX_MAX_IMAGE_EDGE` (1280px). Renders **publish immediately on success — no operator render-review**. Auto-match (FR-014/FR-015) is now an **optional fallback** used only for selection-less render requests. **Sources (ADR-027):** Local = seeded Bogotá SKUs, purchasable, but **placeholder-SVG images so their renders are generic**; Brand = Amazon Berkeley Objects (CC BY 4.0, attributed), real photos, **display-only** ("not sold by Spazio" + "View at retailer", excluded from render-to-purchase). **Not done / mock:** not deployed (render host must be Apple Silicon); real payments unchosen (checkout is mock — ADR-003); Local-supplier real images pending.
>
...
The core promise is that the AI never invents furniture: every rendered item must correspond to a real, purchasable SKU already loaded into the marketplace (PRD §1, BR-6, BR-14; FR-016). *(Scoped caveat — ADR-027, 2026-07-15: this promise holds fully for the **supplier track**. A temporary, clearly-labeled **public-catalog bootstrap fallback** (Amazon Berkeley Objects, CC BY 4.0, attributed) may additionally supply real `source=public` products while no supplier catalog exists; these are real, attributed, non-fabricated, and **display-only** — never added to cart/checkout/orders/commission/MoR — so they sit outside the purchasable-SKU guarantee without weakening it. See the "Public-catalog / bootstrap ingestion adapter" module below and FEAT-017 / FR-062..065 / NFR-019.)*
...
5. **Matching against real inventory.** The system matches real, currently available catalog SKUs to the style, dimensions, budget, and locality. It renders only in-stock ready-made or validly made-to-order products, excludes catalog entries with incomplete required data, keeps total cost within budget plus the agreed tolerance, and never fabricates products (FR-014/FR-016/FR-018/FR-019/FR-021, BR-2/BR-4/BR-6/BR-9/BR-11/BR-14). *(Current built flow inverts this: the user browses and selects up to 3 products which are composited exactly (FR-066–FR-068); auto-match is now an optional fallback for selection-less requests — ADR-028, 2026-07-15; see the "Current system (as built)" summary above.)*
...
9. **Auto cart.** The cart is auto-populated with every product shown in the render. The cart is a suggestion and must be explicitly confirmed before payment; the user can review, remove, or swap items (PRD §8.11–8.12, FR-031/FR-032/FR-033/FR-034/FR-035, BR-31). *(Current built flow — ADR-027/ADR-028, 2026-07-15: only `source=supplier` (Local) render items populate the cart; `source=public` (Brand) items are display-only and excluded (FR-064), so a Brand-only selection yields an empty cart by design.)*
...
|---|---|---|
| Frontend | **Web app** — Next.js 15 (App Router) + React 19 + TypeScript + Tailwind (`web-demo/`), built and verified locally *(supersedes ADR-001's native-iOS/SwiftUI choice per ADR-024, 2026-07-14; the iOS text in ADR-001 below is kept as history)* | ADR-001; **ADR-024** |
...
| **Public-catalog / bootstrap ingestion adapter** *(ADR-027, 2026-07-15; temporary)* | **Distinct from supplier ingestion** (an ADR-006 carve-out — not a supplier self-service channel): fetch/import a subset of the **Amazon Berkeley Objects (ABO)** public dataset (CC BY 4.0) and store `source=public` products with attribution fields (source name/URL, source image URL, image license — data-model spec in `07_data_model.md`). Used as a fallback for matching/render/display **only when no supplier catalog is available**. Introduces **external egress** the closed supplier catalog did not have (the dataset fetch/import path and outbound "View at retailer" links), which must be controlled and recorded (NFR-019). Products are **display-only** and **never** enter cart/checkout/orders/commission/MoR. **Image provenance/attribution flows from the fallback source into the ADR-026 render:** a composited render is a derivative of a CC BY image, so attribution propagates into the stored render and its display (FR-065, NFR-019). Temporary bootstrap track, removable once real suppliers exist without touching the supplier track. | FEAT-017; FR-062, FR-063, FR-064, FR-065 (NFR-019); ADR-027, ADR-026; carve-outs on ADR-004/ADR-006/ADR-007/ADR-014/ADR-023 |
...
| **Catalog browse & selection** *(ADR-028, 2026-07-15)* | Serve a **public** (not device-scoped) browse surface so the user can pick furniture instead of the AI auto-choosing: return the approved, renderable products of a chosen **source** (`supplier`\|`public`) and **style** (optional budget filter), each with an image URL, and stream a product's stored image. Reuses the product-summary shape and the style-matching helper of Product matching (no duplication). Backs the browse → select ≤3 → render flow; the selection (`productIds`, ≤3) is threaded to the render request. Originally delivered with no schema change; since BUG-005 (2026-07-16) the requested selection is also persisted on `RenderRequest.requestedProductIds` (additive migration), so a failed render keeps the attempted products. | FEAT-018; FR-066, FR-067, FR-068, FR-069; ADR-028, ADR-027, ADR-026 |
| **Product matching** *(auto-match now optional — ADR-028)* | Match real, currently available SKUs to style, dimensions, budget, and locality; keep total cost within budget plus tolerance; disclose unmet budgets and offer the closest alternative; suggest similar products or mark items unavailable when no strong match exists. **Under ADR-028 this auto-match is an optional fallback**, run only when a render request carries no user selection (`productIds`); when a selection is present the render composites exactly those products (FR-068). | FEAT-005 (matching side), FEAT-018; FR-014, FR-019, FR-021, FR-022, FR-023 (BR-2, BR-9, BR-10, BR-13); ADR-008, ADR-028 |
...
> **User-curated selection flow (ADR-028, 2026-07-15).** The furnishing flow is inverted from AI auto-furnish (PRD §8) to **browse → select up to 3 → render exactly the selection** (FEAT-018): the new **Catalog browse & selection** surface feeds a `productIds` selection into the render request; the Rendering pipeline composites exactly those validated products (≤3 — matching the ADR-026 Klein reference-image limit), and Product matching's auto-match becomes an optional fallback for selection-less requests. Provenance (ADR-027) is unchanged — a `source=supplier` selection populates the cart, a `source=public` selection is display-only (empty cart). The catalog browse endpoints are public/unauthenticated; NFR-007 device scoping on the project/render routes is preserved.
...
> **Update (#31, PR #32/#33 — post-ADR-024):** this section describes the demo **as originally delivered**. The app has since been wired to the real backend: the wizard runs over `/api/v1` (Next.js rewrite → `backend/`, Fastify + Prisma + Postgres), the cart/checkout/order rows are real, and the catalog is seeded in Postgres (`backend/prisma/seed.ts`). Still mock: the payment capture (ADR-003 vendor open). **Update (ADR-025, 2026-07-14):** renders are **published immediately on generation success — there is no operator-approval wait** (FEAT-016, #38). *(An earlier draft of this note said renders "wait for real operator approval"; that was corrected here — the review gate is retired.)*
>
> **Update (ADR-026 + ADR-028, corrected 2026-07-15) — current built system.** The demo below is described **as originally delivered** (an AI-auto-furnish `room → style → render` wizard with a cached composite). The **current built-and-verified-locally** system differs on two axes: **(1) Flow (FEAT-018/ADR-028):** the flow is now **user-curated browse-and-select** — after source + style the user **browses the real catalog and selects up to ≤3 products**, which are threaded to the render request and composited exactly (auto-match is only a selection-less fallback); the source choice is **Local suppliers** (`source=supplier`, purchasable) vs **Brand suppliers** (`source=public`, display-only, "View at retailer"). **(2) Render engine (ADR-026):** the real engine is **self-hosted FLUX.2 Klein 4B via the mflux CLI** on an Apple-Silicon worker (backend `MfluxRenderPipeline`), selected with `RENDER_ENGINE=mflux`; the default is a `fake`/cached provider that keeps CI hermetic. The legacy `OpenAIRenderProvider`/`IMAGE_API_KEY` stub in `web-demo/` predates ADR-026 and is **not** the engine. See the **"Current system (as built)"** note at the top of this document for the full flow.
...
**Run.** *(As originally delivered — superseded: the current app requires the backend running first; see `web-demo/README.md` "Run it (full stack)" — backend on :3001, then the web app. The standalone/Vercel/`IMAGE_API_KEY` guide below survives only under that README's "Historical" section.)* Node 20+; `cd web-demo`, `npm install`, optionally set `IMAGE_API_KEY` in `.env.local`, then `npm run dev` → `http://localhost:3000`. **Deploy:** Vercel (import the repo, project root = `web-demo`). The full guide is in `web-demo/README.md`.
