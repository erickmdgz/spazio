...
Accepted (demo scope). **Update (ADR-024, 2026-07-14):** the web-app delivery recorded here for the demo is now the product direction — see ADR-024. The mock substitutions below (in-memory state, mock checkout, faked render, seeded catalog) remain demo artifacts slated for replacement by the real backend integration. **Update (ADR-025, 2026-07-14):** the operator render-review gate (FEAT-006 / FR-027) is retired product-wide; the demo's "no operator QA" is no longer a divergence from ADR-002 to be closed by the real-backend integration — renders are published immediately on generation success (ADR-025). **Update (ADR-027, 2026-07-15):** because Spazio has no onboarded suppliers yet, the demo's seeded catalog is **extended with a seeded Amazon Berkeley Objects (ABO) subset** that provides the display-only `source=public` public-catalog track (real products, CC BY 4.0, attributed, clearly labeled "not sold by Spazio" with a "View at retailer" outbound link, never in cart/checkout); the supplier track keeps its seeded fake-supplier data unchanged. This is a dual-track catalog for the demo, not a change to any real product decision. See `decisions/ADR-027_public-catalog-bootstrap-fallback.md`. **Update (2026-07-15/16 — ADR-026/028/029):** the ADR-024 backend integration has since **landed**: the in-memory state, the faked demo render, and the seeded in-code catalog were replaced by the real backend (Postgres state, real render jobs via `RENDER_ENGINE=fake|mflux`, seeded backend catalog); of the mock substitutions only the **mock checkout** remains, now an *accepted* demo-scope limitation (ADR-029), no longer "slated for replacement".
...
- **Checkout:** mock only. No real payment and no settlement; checkout groups the order by supplier (one PO each) and shows a mock "Pay $ X" button (amount in COP). Per **ADR-022**, checkout collects minimal contact details (email/phone/address) with **no accounts**.
...
**Routes/flow:** `/` (landing) → `/room` (pick a sample living/bedroom or upload → prepared result; approximate dimensions) → `/style` (Modern Mediterranean / Warm Minimalist / Scandinavian + free text + COP budget slider 2,000,000–12,000,000) → `/render` (simulated generate, then a furnished render with tappable product hotspots + budget indicator, 10% tolerance) → product detail sheet (`ProductSheet.tsx`) → `/cart` (items, per-item + total COP, budget-vs-total, remove/swap) → `/checkout` (minimal contact per ADR-022, order grouped by supplier, one PO each, mock "Pay $ X" button, amount in COP) → `/confirmation` (order number, per-supplier breakdown, per-item delivery/production dates, operator-in-the-loop message).
