...
> This document is a first structuring of Spazio's domain entities, derived from the PRD v0.7 and the One-Week iOS Pilot. *(Correction, 2026-07-15: the former blanket "Nothing here is implemented" is no longer accurate.)* **Update (PR #21):** the pilot entity subset is now expressed as a Prisma schema in `backend/prisma/schema.prisma` and is **built and verified on a local stack (not deployed, no `vX.Y.Z` release)**; the entities and fields backing the current flow — `Project`, `RoomPhoto` (byte upload), `RenderRequest`/`Render`/`RenderItem`, `Cart`/`CartItem`, `Order`/`PurchaseOrder`, `Product` (incl. `source`), `Operator`, `Style` — exist in that schema, while the remaining entities/fields here (and the full-product draft) stay specification only. The physical schema, database engine, and field types follow the technology stack — a human decision now decided for the pilot as a native iOS (SwiftUI) app with a small managed backend and a managed relational (Postgres) database plus object storage, single environment/region (see `ADR-001 Technology stack`; the client has since changed to the **web app** — `ADR-024`, 2026-07-14). **Update (ADR-025, 2026-07-14):** the operator render-review gate is removed — renders are published to the requesting user immediately on generation success; the `Render` review states and reviewer stamps below are marked Retired (removed from the as-built code by FEAT-016 (#38)). **Update (ADR-026, 2026-07-14):** the render engine is now self-hosted FLUX.2 Klein 4B run locally via the mflux CLI (superseding ADR-002's hosted-image-API clause; no structural change to the model below — `RenderRequest.inference_cost` now records local-compute cost rather than a third-party call). Treat every table below as a specification to review, not a settled design.
...
> **User-curated selection note (ADR-028, 2026-07-15 — current flow).** In the current system a render request carries an **optional user selection of up to 3 product ids** (API `POST /renders` `productIds`; FR-066–FR-068). When present, the resulting `Render` composites **exactly** those validated products (each becomes a `RenderItem`), and auto-match (FR-014/FR-015) is **not** run; when absent, auto-match runs as the **optional fallback** (backward compatible). The >3 cap is enforced server-side (FR-067; matches the FLUX.2 Klein ~2–3 reference-image limit, ADR-026). The composited set is persisted as `RenderItem` rows; **since BUG-005 (2026-07-16) the selection as requested is also snapshotted on the request row itself** (`requested_product_ids` below) — `RenderItem` rows exist only for successful renders and the queue payload is volatile, so without the snapshot a **failed** render lost which products the user attempted. "Try other furniture" (FR-069) is an ordinary new render request on the same `Project`, reusing the same photo/source/style with a different selection.
...
| requested_product_ids | UUID[] | Yes (default `[]`) | **(As built — BUG-005, 2026-07-16.)** Snapshot of the user's curated selection **as requested** (`requestedProductIds` in `schema.prisma`; ADR-028/FR-068), persisted at request creation so a **failed** render keeps the attempted selection (the queue payload is volatile and `RenderItem` rows exist only on success). Empty = auto-match fallback (FR-014/FR-015). |
...
| inference_cost | Decimal | No | Tracked cost per render **(proposed)** (NFR-005); a global inference-cost threshold applies (NFR-003). *(ADR-026, 2026-07-14: the engine is self-hosted (FLUX.2 Klein 4B via mflux), so this is now **local-compute cost with near-zero marginal per-render vendor fee**, not a metered third-party call; the field and threshold semantics are otherwise unchanged.)* |
...
| status | Enum | Yes | `completed` / `failed` **(proposed values)**; the render is published to the user immediately on generation success. *(Superseded by ADR-025, 2026-07-14: review states `pending_review`/`approved`/`rejected` removed; removed from code by FEAT-016 (#38).)* *(As-built note, 2026-07-15: the pilot `schema.prisma` `Render` model carries **no `status` column** — generation status lives on `RenderRequest.status` (`queued`/`processing`/`completed`/`failed`) and `Render.imageKey` is nullable until the pipeline completes.)* |
| reviewed_by | UUID (FK) | No | **Retired — ADR-025 (2026-07-14):** render review removed. *(As-built, 2026-07-15: already removed from `schema.prisma` by FEAT-016 (#38) — the `Render` model has no `reviewedBy` field; this is not pending removal. Was built in PR #27, stamped from the operator session.)* |
| reviewed_at | DateTime | No | **Retired — ADR-025 (2026-07-14):** render review removed. *(As-built, 2026-07-15: already removed from `schema.prisma` by FEAT-016 (#38) — the `Render` model has no `reviewedAt` field; this is not pending removal. Was built in PR #27.)* |
